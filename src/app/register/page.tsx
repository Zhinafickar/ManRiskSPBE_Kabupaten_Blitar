...
export default function RegisterPageContent() {
    ...
    try {
      if (!isFirebaseConfigured || !auth || !db) {
        toast({
          ...
          description:
            'Please add your Firebase credentials to the .env file to enable registration.',
        });
        setIsLoading(false);
        return;
      }
      ...
      if (
        !ADMIN_ROLES.includes(values.role) &&
        values.role !== 'Penguji Coba'
      ) {
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        description =
          'Registration failed due to a database permission error. Please check Firestore rules.';
      } else {
  ...
  return (
    ...
    >
      <Card className="w-full max-w-md shadow-2xl bg-white bg-opacity-95 backdrop-blur-md">
                    ...
                    <Popover open={open} onOpenChange={setOpen}>
                      <PopoverTrigger asChild>
                        <FormControl>
                          ...
                          >
                            {field.value ? availableRoles.find((role) => role === field.value) : 'Select your OPD'}
